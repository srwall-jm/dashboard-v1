...
const App: React.FC = () => {
  ...
  const [isSidebarOpen, setIsLoadingSidebarOpen] = useState(false);
  ...
  // Fix: Defined isAnythingLoading to handle GA4 and GSC data fetching states
  const isAnythingLoading = isLoadingGa4 || isLoadingGsc;

  // Fix: Implemented handleGenerateInsights to call the Gemini service for data analysis
  const handleGenerateInsights = async () => {
    ...
    try {
      const summary = `
        Contexto del Dashboard: ${activeTab}
        País Seleccionado: ${filters.country}
        Tipo de Query: ${filters.queryType}
        Rango de Fechas: ${filters.dateRange.start} a ${filters.dateRange.end}
        
        Resumen de Performance Orgánica:
        - Sesiones: ${channelStats.organic.current.sessions}
        - Conversión: ${channelStats.organic.current.cr.toFixed(2)}%
        - Ingresos: €${channelStats.organic.current.revenue.toLocaleString()}
        
        Resumen de Performance de Pago:
        - Sesiones: ${channelStats.paid.current.sessions}
        - Conversión: ${channelStats.paid.current.cr.toFixed(2)}%
        - Ingresos: €${channelStats.paid.current.revenue.toLocaleString()}
        
        Datos de Keyword: Se han analizado ${filteredKeywordData.length} palabras clave.
      `;
      const result = await getDashboardInsights(summary, activeTab);
      setAiInsights(result || "No se pudo obtener el análisis de la IA.");
    } catch (e) {
      console.error("Error generating insights:", e);
      setAiInsights("Error al conectar con el servicio de IA.");
    } finally {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-white text-center">
        ...
        <h1 className="text-3xl font-black mb-4">SEO & SEM Advanced Reporting Suite</h1>
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col md:flex-row">
      <aside className="w-80 bg-slate-950 text-white flex flex-col hidden xl:flex border-r border-white/5">
        <div className="p-8 flex-1">
          <div className="flex items-center gap-3 mb-10"><Activity className="text-indigo-500 w-8 h-8" /><div><h1 className="font-black">SEO Master</h1><p className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest">Advanced Suite</p></div></div>
      ...
      <main className="flex-1 p-5 md:p-10 overflow-x-hidden">
        ...
        {/* AI Insights display */}
        {aiInsights && (
          <div className="mt-12 bg-indigo-50 border border-indigo-200 p-8 rounded-[32px] animate-in fade-in slide-in-from-top-4">
            <div className="flex items-center gap-3 mb-6">
              <Sparkles className="w-5 h-5 text-indigo-600" />
              <h3 className="text-lg font-black text-indigo-900">IA Insights & Recomendaciones</h3>
            </div>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6">
      ...
      <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
        <div className="flex justify-between items-center mb-8">
          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tendencia de Sesiones</h4>
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={trendData}>
              ...
              <Area name="Organic" type="monotone" dataKey="organicSessions" stroke="#6366f1" strokeWidth={3} fillOpacity={0.1} fill="#6366f1" />
              <Area name="Paid" type="monotone" dataKey="paidSessions" stroke="#f59e0b" strokeWidth={3} fillOpacity={0.1} fill="#f59e0b" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
        <div className="flex justify-between items-center mb-8">
          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tendencia de Revenue (€)</h4>
        </div>
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={trendData}>
              <defs>
                 <linearGradient id="rOrg" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.1}/><stop offset="95%" stopColor="#10b981" stopOpacity={0}/></linearGradient>
                 <linearGradient id="rPaid" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f43f5e" stopOpacity={0.1}/><stop offset="95%" stopColor="#f43f5e" stopOpacity={0}/></linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
              <XAxis dataKey="date" tick={{fontSize: 9, fontWeight: 700}} axisLine={false} tickLine={false} />
              <YAxis axisLine={false} tickLine={false} tick={{fontSize: 9}} />
              <Tooltip formatter={(val: number) => `€${val.toLocaleString()}`} contentStyle={{borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)'}} />
              <Legend verticalAlign="top" align="center" iconType="circle" />
              <Area name="Organic Revenue" type="monotone" dataKey="organicRevenue" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#rOrg)" />
              <Area name="Paid Revenue" type="monotone" dataKey="paidRevenue" stroke="#f43f5e" strokeWidth={3} fillOpacity={1} fill="url(#rPaid)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
           <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-10">Embudo de Conversión Ecommerce</h4>
           <div className="space-y-8">
              {funnelData.organic.map((stage: any, i: number) => {
                ...
                const oRate = i === 0 ? '100' : (oVal / funnelData.organic[0].value * 100).toFixed(1);
                const pRate = i === 0 ? '100' : (pVal / funnelData.paid[0].value * 100).toFixed(1);

                return (
                  <div key={stage.name}>
                    <div className="flex justify-between items-end mb-3">
                       <span className="text-xs font-black uppercase text-slate-700 tracking-tight">{stage.name}</span>
                       <div className="flex gap-6 text-[10px] font-bold">
                          <span className="text-indigo-600">ORG: {oVal.toLocaleString()} ({oRate}%)</span>
                          <span className="text-amber-600">PAID: {pVal.toLocaleString()} ({pRate}%)</span>
                       </div>
                    </div>
                    <div className="h-5 bg-slate-100 rounded-full overflow-hidden flex shadow-inner">
                       <div className="h-full bg-indigo-500 transition-all duration-700 relative" style={{ width: `${(oVal / max) * 50}%` }}>
                          <div className="absolute inset-0 bg-white/10"></div>
                       </div>
                       <div className="h-full bg-amber-500 transition-all duration-700 relative" style={{ width: `${(pVal / max) * 50}%` }}>
...
const KpiContainer = ({ label, color, s, comparisonEnabled }: any) => (
  <div className="space-y-4">
    <div className="flex items-center gap-2 px-2"><div className={`w-2.5 h-2.5 rounded-full bg-${color}-500`} /><h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest">{label}</h4></div>
    <div className="grid grid-cols-2 gap-4">
      ...
      <KpiCard title="Revenue" value={`€${s.current.revenue.toLocaleString()}`} comparison={comparisonEnabled ? s.changes.revenue : undefined} absoluteChange={comparisonEnabled ? s.abs.revenue : undefined} icon={<Tag />} prefix="€" color="emerald" />
  ...
  return (
    <div className="bg-white rounded-[32px] border border-slate-200 overflow-hidden shadow-sm">
      <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/50">
        <div><h3 className="text-xl font-black">URLs Deep Dive</h3><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Keywords por landing page</p></div>
        <div className="relative w-full md:w-80"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4" /><input type="text" placeholder="Filtrar URL..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-medium outline-none" /></div>
      </div>
