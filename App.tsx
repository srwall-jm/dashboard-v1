...
const App: React.FC = () => {
  ...
  const [globalKeywordBridgeDataGoogleAds, setGlobalKeywordBridgeDataGoogleAds] = useState<KeywordBridgeData[]>([]);
  ...
  const fetchGoogleAdsKeywords = async () => {
    ...
    try {
        ...
        // Build keyword bridge rows for Search Efficiency.
        ...
        // Build global aggregate rows by keyword.
        ...
        googleAdsKwResults.forEach(item => {
            ...
            globalKwBridgeMap[cleanKw].paidClicks += item.paidClicks;
            globalKwBridgeMap[cleanKw].paidConversions += item.paidConversions;
        ...
        const finalGlobalKwResults: KeywordBridgeData[] = Object.values(globalKwBridgeMap).map((item: any) => {
            const avgRank = item.organicRankCount > 0 ? item.organicRankSum / item.organicRankCount : null;
            const paidSessions = item.paidSessions || 0;
            const paidConversions = item.paidConversions || 0;
            const paidCvr = paidSessions > 0 ? (paidConversions / paidSessions) * 100 : 0;
            const avgCpc = paidSessions > 0 ? item.ppcCost / paidSessions : (googleAdsGlobalMetrics?.avgCpc || 0);
            ...
            return {
                ...
                paidCta: paidCvr,
                avgCpc,
                actionLabel: avgRank !== null && avgRank <= 3 && paidSessions > 0 ? "REVIEW" : "MAINTAIN"
            };
        });
        ...
        <div className="w-full">
          ...
          {activeTab === DashboardTab.PPC_SEO_BRIDGE && (
            <SeoPpcBridgeView 
                ...
                googleAdsKeywordData={globalKeywordBridgeDataGoogleAds.length > 0 ? globalKeywordBridgeDataGoogleAds : keywordBridgeDataGoogleAds}
