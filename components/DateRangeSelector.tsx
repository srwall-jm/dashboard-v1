...
export const DateRangeSelector: React.FC<{
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      // Defensive check: Ensure ref and current exist before accessing
  ...
  const ranges = [
    ...
    { label: 'This month', getValue: () => { const d = new Date(); const start = new Date(d.getFullYear(), d.getMonth(), 1); const endOfMonth = new Date(d.getFullYear(), d.getMonth() + 1, 0); return { start, end: endOfMonth > d ? d : endOfMonth }; } },
    { label: 'This month to date', getValue: () => { const d = new Date(); return { start: new Date(d.getFullYear(), d.getMonth(), 1), end: d }; } },
    { label: 'This quarter', getValue: () => { const d = new Date(); const q = Math.floor(d.getMonth() / 3); const start = new Date(d.getFullYear(), q * 3, 1); const endOfQuarter = new Date(d.getFullYear(), (q + 1) * 3, 0); return { start, end: endOfQuarter > d ? d : endOfQuarter }; } },
    ...
    { label: 'This year', getValue: () => { const d = new Date(); const start = new Date(d.getFullYear(), 0, 1); const endOfYear = new Date(d.getFullYear(), 11, 31); return { start, end: endOfYear > d ? d : endOfYear }; } },
